...
/**
 * Collects concepts from ValueSets and CodeSystems based on compose definitions.
 */
public class ConceptCollector {
     ...
     */
    private void processIncludes(ValueSet sourceValueSet,
                                ValueSetExpansionComponent expansion,
                                ExpansionRequest request,
        ...
        for (int i = 0; i < includes.size(); i++) {
            ...
            if (include.hasSystem()) {
                processSystemInclude(include, sourceValueSet, expansion, request, 
                                   includedCodes, excludedSystems, systemVersionMap, 
     ...
     */
    private void processSystemInclude(ConceptSetComponent include,
                                     ValueSet sourceValueSet,
                                     ValueSetExpansionComponent expansion,
                                     ExpansionRequest request,
                                     List<ValueSetExpansionContainsComponent> includedCodes,
        ...
        String version = determineSystemVersion(systemUrl, include.getVersion(), 
                                               systemVersionMap, checkSystemVersionMap, 
                                               forceSystemVersionMap);
     ...
     */
    private void processValueSetInclude(ConceptSetComponent include,
                                       ValueSetExpansionComponent expansion,
                                       ExpansionRequest request,
                                       List<ValueSetExpansionContainsComponent> includedCodes) {
        ...
        for (CanonicalType valueSetToInclude : include.getValueSet()) {
            ...
            if (importedVs != null && importedVs.hasUrl() && importedVs.hasVersion()) {
                ...
                expansion.addParameter()
                    ...
                    .setValue(resolvedCanonical.copy());
    ...
    private void processExcludes(ValueSet sourceValueSet,
        ...
        for (ConceptSetComponent exclude : sourceValueSet.getCompose().getExclude()) {
            if (exclude.hasSystem()) {
                ...
                CodeSystem codeSystem = resourceFinder.findCodeSystem(
                    systemUrl, exclude.getVersion(), request);
        ...
        if (!codesToExclude.isEmpty()) {
            includedCodes.removeIf(component -> 
                codesToExclude.contains(component.getSystem() + "|" + component.getCode())
            );
        }
    }
     ...
     */
    private void collectExcludedCodes(Set<String> codesToExclude,
                                     CodeSystem codeSystem,
                                     ConceptSetComponent exclude,
                                     String systemUrl) {
        
        // Handle is-not-a filter
        Optional<ConceptSetFilterComponent> isNotAFilter = exclude.getFilter().stream()
            .filter(f -> "concept".equals(f.getProperty()) && 
                       f.getOp() == FilterOperator.ISNOTA)
            .findFirst();
        
        if (isNotAFilter.isPresent()) {
            String codeToExclude = isNotAFilter.get().getValue();
            CodeSystem.ConceptDefinitionComponent conceptToExclude = 
                resourceFinder.findConceptInCodeSystem(codeSystem, codeToExclude);
        ...
        List<ConceptSetFilterComponent> propertyFilters = exclude.getFilter().stream()
            .filter(f -> f.getOp() != FilterOperator.ISNOTA)
            .collect(Collectors.toList());
        
        if (!propertyFilters.isEmpty()) {
            collectExcludedCodesRecursive(codesToExclude, codeSystem, 
                                        codeSystem.getConcept(), propertyFilters);
        }
     ...
     */
    private void addCodesFromCodeSystem(List<ValueSetExpansionContainsComponent> allCodes,
                                       ValueSet sourceValueSet,
                                       CodeSystem codeSystem,
                                       ConceptSetComponent include,
                                       ExpansionRequest request) {
     ...
     */
    private void processExplicitConcepts(List<ValueSetExpansionContainsComponent> allCodes,
                                        ValueSet sourceValueSet,
                                        CodeSystem codeSystem,
                                        ConceptSetComponent include,
                                        ExpansionRequest request) {
        ...
        for (ConceptReferenceComponent conceptRef : include.getConcept()) {
            ...
            if (conceptDef != null &&
                conceptFilter.matchesAllFilters(conceptDef, include.getFilter(), codeSystem) &&
                conceptFilter.shouldIncludeConcept(sourceValueSet, conceptDef, 
                                                  conceptRef.getDisplay(), request)) {
                
                allCodes.add(componentBuilder.createExpansionComponent(
                    codeSystem, conceptDef, request));
            }
        }
    }
     ...
     */
    private void processFilteredConcepts(List<ValueSetExpansionContainsComponent> allCodes,
                                        ValueSet sourceValueSet,
                                        CodeSystem codeSystem,
                                        ConceptSetComponent include,
                                        ExpansionRequest request) {
        ...
        List<ConceptSetFilterComponent> propertyFilters = filters.stream()
            .filter(f -> f.getOp() != FilterOperator.ISA && 
                       f.getOp() != FilterOperator.DESCENDENTOF && 
                       f.getOp() != FilterOperator.GENERALIZES)
            .collect(Collectors.toList());
        
        List<ValueSetExpansionContainsComponent> conceptsForThisInclude = new ArrayList<>();
        
        if (generalizesFilter.isPresent()) {
            processGeneralizes(conceptsForThisInclude, codeSystem, 
                              generalizesFilter.get().getValue(), propertyFilters, request);
        } else {
            processHierarchicalFilters(conceptsForThisInclude, sourceValueSet, codeSystem,
                                      isAFilter, descendentOfFilter, propertyFilters, request);
        }
     ...
     */
    private void processHierarchicalFilters(List<ValueSetExpansionContainsComponent> concepts,
                                           ValueSet sourceValueSet,
                                           CodeSystem codeSystem,
                                           Optional<ConceptSetFilterComponent> isAFilter,
                                           Optional<ConceptSetFilterComponent> descendentOfFilter,
                                           List<ConceptSetFilterComponent> propertyFilters,
                                           ExpansionRequest request) {
        
        List<CodeSystem.ConceptDefinitionComponent> startingConcepts = 
            new ArrayList<>(codeSystem.getConcept());
        
        if (isAFilter.isPresent()) {
            String parentCode = isAFilter.get().getValue();
            CodeSystem.ConceptDefinitionComponent parentConcept = 
                resourceFinder.findConceptInCodeSystem(codeSystem, parentCode);
        ...
        for (CodeSystem.ConceptDefinitionComponent conceptDef : startingConcepts) {
            processConceptRecursive(concepts, sourceValueSet, codeSystem, 
                                  conceptDef, request, propertyFilters);
        }
    }
    ...
    private void processConceptRecursive(List<ValueSetExpansionContainsComponent> parentContainsList,
                                        ...
                                        CodeSystem.ConceptDefinitionComponent conceptDef,
                                        ExpansionRequest request,
                                        List<ConceptSetFilterComponent> vsFilters) {
        ...
        if (conceptDef.hasConcept()) {
            for (CodeSystem.ConceptDefinitionComponent child : conceptDef.getConcept()) {
                processConceptRecursive(listForChildren, sourceValueSet, codeSystem, 
                                      child, request, vsFilters);
            }
        }
    }
     ...
     */
    private void processGeneralizes(List<ValueSetExpansionContainsComponent> concepts,
                                   CodeSystem codeSystem,
                                   String startCode,
                                   List<ConceptSetFilterComponent> propertyFilters,
                                   ExpansionRequest request) {
        
        Map<CodeSystem.ConceptDefinitionComponent, CodeSystem.ConceptDefinitionComponent> 
            parentMap = buildParentMap(codeSystem);
        
        CodeSystem.ConceptDefinitionComponent currentConcept = 
            resourceFinder.findConceptInCodeSystem(codeSystem, startCode);
     ...
     */
    private Map<CodeSystem.ConceptDefinitionComponent, CodeSystem.ConceptDefinitionComponent> 
            buildParentMap(CodeSystem codeSystem) {
     ...
     */
    private void collectDescendantsAndSelf(CodeSystem.ConceptDefinitionComponent concept,
     ...
     */
    private void collectExcludedCodesRecursive(Set<String> codesToExclude,
                                              CodeSystem codeSystem,
    ...
    private Optional<ConceptSetFilterComponent> findFilter(List<ConceptSetFilterComponent> filters,
        ...
        return filters.stream()
            .filter(f -> "concept".equals(f.getProperty()) && f.getOp() == operator)
            .findFirst();
    }
    ...
    private Set<String> getExcludedSystems(List<CanonicalType> excludeSystemParams) {
        ...
        return excludeSystemParams.stream()
    ...
    private String determineSystemVersion(String system,
    ...
    private void validateFilters(ConceptSetComponent include,
                                int includeIndex,
                                ValueSet sourceValueSet) {
        ...
        for (int filterIndex = 0; filterIndex < filters.size(); filterIndex++) {
            ...
            if (Utilities.noString(filter.getValue())) {
                createAndThrowInvalidFilterException(include, includeIndex, 
                                                    filter, filterIndex, sourceValueSet);
            }
        }
    }
    
    private void createAndThrowInvalidFilterException(ConceptSetComponent include,
                                                     int includeIndex,
        ...
        String errorMessage = String.format(
            "The system %s filter with property = %s, op = %s has no value",
            include.getSystem(),
            filter.getProperty(),
            filter.getOp() != null ? filter.getOp().toCode() : "null"
        );
        ...
        String location = String.format(
            "ValueSet[%s|%s].compose.include[%d].filter[%d].value",
            sourceValueSet.getUrl(),
            sourceValueSet.getVersion(),
            includeIndex,
            filterIndex
        );
    ...
    private void addSystemParameters(ValueSetExpansionComponent expansion,
                                    CodeSystem codeSystem,
                                    String systemUrl,
                                    String version) {
    ...
    private boolean hasParameter(ValueSetExpansionComponent expansion,
                                String name,
                                Type value) {
        return expansion.getParameter().stream()
            .anyMatch(p -> name.equals(p.getName()) && value.equals(p.getValue()));
    }
    
    private void addWarningParameter(ValueSetExpansionComponent expansion,
                                    String warningType,
                                    Type value) {
    ...
    private String getStandardsStatus(DomainResource resource) {
        final String EXT_STANDARDS_STATUS = 
            "http://hl7.org/fhir/StructureDefinition/structuredefinition-standards-status";
        
        return resource.getExtension().stream()
            .filter(ext -> EXT_STANDARDS_STATUS.equals(ext.getUrl()) && 
                         ext.getValue() instanceof CodeType)
            .map(ext -> ((CodeType) ext.getValue()).getCode())
            .findFirst()
            .orElse(null);
    }
}
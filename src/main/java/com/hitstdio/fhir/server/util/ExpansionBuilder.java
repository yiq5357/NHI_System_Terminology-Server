...
public class ExpansionBuilder {
    ...
    private static final String EXT_EXPANSION_PROPERTY = 
        "http://hl7.org/fhir/5.0/StructureDefinition/extension-ValueSet.expansion.property";
    private static final String EXT_STANDARDS_STATUS = 
        "http://hl7.org/fhir/StructureDefinition/structuredefinition-standards-status";
    
    private final ConceptFilter conceptFilter;
    
    public ExpansionBuilder(ConceptFilter conceptFilter) {
     ...
     */
    public void buildExpansion(ValueSetExpansionComponent expansion,
     ...
     */
    private void addExpansionParameters(ValueSetExpansionComponent expansion,
                                       ValueSet sourceValueSet,
                                       ExpansionRequest request) {
        ...
        if ("withdrawn".equals(vsStatus)) {
            ...
            expansion.addParameter()
                ...
                .setValue(new UriType(vsCanonical));
        }
        ...
        if (request.getIncludeDesignations() != null && !request.getIncludeDesignations().isEmpty()) {
            expansion.addParameter()
                ...
                .setValue(request.getIncludeDesignations());
        }
        ...
        if (request.getActiveOnly() != null && !request.getActiveOnly().isEmpty()) {
            expansion.addParameter()
                .setName("activeOnly")
                .setValue(request.getActiveOnly());
        }
     ...
     */
    private void addDisplayLanguageParameter(ValueSetExpansionComponent expansion,
                                            ValueSet sourceValueSet,
                                            ExpansionRequest request) {
        ...
        if (displayLanguage != null && !displayLanguage.isEmpty()) {
            expansion.addParameter()
                ...
                .setValue(new CodeType(displayLanguage));
        }
    }
     ...
     */
    private void addVersionParameters(ValueSetExpansionComponent expansion,
                                     ExpansionRequest request) {
     ...
     */
    private void addPropertyDeclarations(ValueSetExpansionComponent expansion,
                                        ValueSet sourceValueSet,
                                        ExpansionRequest request) {
        ...
        if (!sourceValueSet.getCompose().getIncludeFirstRep().hasSystem()) {
            return;
        }
        ...
        if (txResources.containsKey(systemUrl)) {
            Resource resource = txResources.get(systemUrl);
     ...
     */
    private void addPropertyDeclaration(ValueSetExpansionComponent expansion,
                                       String code,
                                       String uri) {
        Extension expansionPropertyExt = expansion.addExtension();
        expansionPropertyExt.setUrl(EXT_EXPANSION_PROPERTY);
        
        expansionPropertyExt.addExtension()
            .setUrl("code")
            .setValue(new CodeType(code));
        
        expansionPropertyExt.addExtension()
            .setUrl("uri")
            .setValue(new UriType(uri));
    }
